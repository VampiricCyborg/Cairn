"""Tests for cairn.providers.openai.OpenAIProvider.
...
def _provider(client: MagicMock, *, api_key: str | None = "test-key") -> OpenAIProvider:
...
def test_valid_json_response_converts_to_staged_entries() -> None:
    ...
    assert request.kwargs["json"]["model"] == "llama-3.1-8b-instant"
    assert request.kwargs["json"]["response_format"]["type"] == "json_schema"
...
def test_missing_api_key_raises_provider_unavailable_without_http_call() -> None:
    client = MagicMock()

    with pytest.raises(ProviderUnavailableError, match="api_key_env"):
        _provider(client, api_key=None).extract(_reflectable_trace(), known=[], max_candidates=3)

    client.post.assert_not_called()


def test_401_response_raises_provider_unavailable() -> None:
    client = _client(_chat_response("", status_code=401))
