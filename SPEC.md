...
|---|---|---|
| `candidate` → `dropped` | Fails schema validation (checked upstream, by the provider), or matches a `rejected/` tombstone (`Curator.check_tombstoned`). | Never written to `staging/`. |
| `candidate` → `staged` | Passes schema and isn't tombstoned. A near-duplicate of an approved or staged entry (`Curator.find_near_duplicate`) is marked `proposed_amendment_of` rather than blocked; a structural contradiction against an approved entry (`Curator.find_contradiction`) is marked `proposed_supersession_of`. Neither drops the candidate — both still stage, for a human to decide. | Written to `staging/`, `status: staged`. |
...
| `staged` → `rejected` | Human rejects with a reason. | Written to `rejected/<id>.json` as a tombstone (`id`, `title`, `reason`, `excerpt_sha256`). Not re-proposed. |
...
The tombstone carries `excerpt_sha256`, copied from the rejected candidate's
own evidence, alongside `id`, `title`, and `reason`. `Curator.check_tombstoned`
uses it to catch an exact re-proposal of the same transcript excerpt even
under a freshly generated `id`, and falls back to a title-similarity check
for a re-derived lesson from a different session's excerpt (which never
shares a hash with the original). This, plus the near-duplicate check
running against `staging/` as well as `entries/`, is what closes the
staging-level duplicate-suppression gap the reflect e2e tests surfaced — see
`cairn.core.curator.Curator.stage_candidate`.
...
| `proposed_amendment_of` | string or null | no (default `null`) | Set by the Curator on a staged candidate whose title (or, in the gray zone, body) near-duplicates an approved or already-staged entry: that entry's id. Approving the candidate amends the target in place instead of creating a new entry. |
| `proposed_supersession_of` | string or null | no (default `null`) | Set by the Curator on a staged candidate that structurally looks like it may contradict an approved entry (same `scope` and `type`, distinct title, overlapping `tags`): that entry's id. A structural flag only — the human reviewer decides whether it actually supersedes. |
