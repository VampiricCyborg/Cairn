...
| `staging/` | yes | Candidate entries awaiting a human decision. Same file shape as `entries/`, with `status: staged`. |
| `rejected/` | yes | Tombstones: an entry's id, content hash, and rejection reason. Suppresses re-proposal of the same candidate. |
...
| `candidate` → `staged` | Passes schema, near-duplicate, contradiction, and scope gates. | Written to `staging/<type>/`, `status: staged`. |
...
| `staged` → `rejected` | Human rejects with a reason. | Written to `rejected/` as a tombstone: id, content hash, reason. Not re-proposed. |
